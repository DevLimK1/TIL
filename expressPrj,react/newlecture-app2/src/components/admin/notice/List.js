...
class List extends React.Component {
  ...
  pagerClick({num}){
    ...
    let f=this.state.fieldSelected;

    //방법2) query-string 쓰는 방법
    // let {f="title",q=""}= queryString.parse(this.props.location.search); //객체로 속성이 들어간다.-> import queryString from 'query-string'; 필요, 기본값 설정
    ...
    // let field=?;
    // let query=?;
    this.props.history.push(`/customer/notice/list?p=${num}&f=${f}&q=${q}`);
