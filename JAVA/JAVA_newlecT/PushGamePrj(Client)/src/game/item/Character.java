...
public class Character implements Movable {
	private Image bearImg_left, bearImg_right, wallImg, sunImg; // 곰, 빙산 이미지
	...
	public Character() {
		...
		try {
			...
			wallImg = ImageIO.read(new File("res/iceice.png")); // 빙산 이미지
			sunImg = ImageIO.read(new File("res/sunrise.png")); // 해 이미지
//			sunImg = ImageIO.read(new File("res/sunrise2.png")); // 강사님 이미지
		} catch (IOException e) {
...
//		heightR = bearImg_right.getHeight(FightCanvas.getInstacne());
	...
	public void draw(Graphics g) {
		...
		int offsetX = 183 / 2; // 왼쪽 이미지의 x축 중심점 이동위해서 선언
		int offsetY = 400 / 2; // 왼쪽 이미지의 y축 중심점 이동위해서 선언
